...
int main(int ac, char **av)
{
	...
	while ((c = getopt_long(ac, av, shortopts, opts, NULL)) != -1) {
		...
		case 'p': /* --preferred */
			...
			break;
                case 'O': /* --preferred-order */
                        checknuma();
                        nopolicy();
                        setpolicy(MPOL_PREFERRED_ORDER);
                        if (parse_all)
                            mask = numactl_parse_nodestring(optarg, ALL);
                        else
                            mask = numactl_parse_nodestring(optarg, CPUSET);
                        ...
                        if (shmfd >= 0) {
                            ...
                            if (set_mempolicy(MPOL_PREFERRED_ORDER, mask->maskp, mask->size + 1) < 0)
                                complain("set_mempolicy MPOL_PREFERRED_ORDER: %s\n", strerror(errno));
                        }
                        numa_bitmask_free(mask);
                        checkerror("setting preferred-order policy");
                        break;
		case 'l': /* --local */
			checknuma();
